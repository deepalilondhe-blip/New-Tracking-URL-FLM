...
console.log('📋 Running PPC-FS Campaign in iPad (Tablet) Emulation...');
...
(async () => {
    ...
    try {
        ...
        console.log('✅ iPad PPC-FS Test completed:', result);

        await context.tracing.stop({ path: path.join(traceDir, 'PPC_FS_iPad.zip') });
        console.log(`\n📋 Fixed Tablet Trace Report saved`);

    } catch (error) {
        console.error('❌ iPad PPC-FS Test failed:', error);
    } finally {
