...
process.env.OVERRIDE_SLIDER = "50000 More";
process.env.OVERRIDE_STATE = "Colorado";
process.env.OVERRIDE_PHONE = "719-594-8642";
process.env.PROCESS_BROWSER = "firefox";
process.env.PROCESS_LABEL = "W-Firefox";

console.log('✅ Running PPC-M/CA Campaign with EXACT VALUES ONLY (NO ROTATION)...');
...
console.log('✅ Slider Value: 50000 More (EXACT)');
console.log('✅ State: Colorado (EXACT)');
console.log('✅ Phone: 719-594-8642 (EXACT)');
...
(async () => {
    ...
    try {
        ...
        console.log('✅ PPC-M/CA Test completed:', result);

        await context.tracing.stop({ path: path.join(traceDir, 'PPC_M_CA_EXACT.zip') });
        console.log(`\n📋 Trace Report saved to: traces/PPC_M_CA_EXACT.zip`);

    } catch (error) {
        console.error('❌ PPC-M/CA Test failed:', error);
    } finally {
