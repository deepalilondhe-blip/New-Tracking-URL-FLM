...
console.log('📋 Running PPC-CR Campaign in iPad (Tablet) Emulation...');
...
(async () => {
    ...
    try {
        ...
        console.log('✅ iPad PPC-CR Test completed:', result);

        await context.tracing.stop({ path: path.join(traceDir, 'PPC_CR_iPad.zip') });
        console.log(`\n📋 Fixed Tablet Trace Report saved`);

    } catch (error) {
        console.error('❌ iPad PPC-CR Test failed:', error);
    } finally {
